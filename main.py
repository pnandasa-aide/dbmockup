...
def verify_schema(db, table_schema):
    ...
    actual_columns_list = db.get_table_columns(table_name)
...
def process_table(db, gen, table_schema, profile, batch_size):
    table_name = table_schema['table_name']
    print(f"\nProcessing table: {table_name}")
    
    # 1. Verify schema - will exit if mismatch
    pk_col = verify_schema(db, table_schema)

    # 2. Journaling info
    j_info = db.get_journal_info(table_name)
    if j_info:
        ...
        je_info = db.get_journal_entries_info(table_name)
        if je_info:
    ...
    try:
        ...
        for table_schema in schema_data['tables']:
            ...
            if table_name in profiles:
                process_table(db, gen, table_schema, profiles[table_name], config['settings'].get('batch_size', 1000))
            else:
